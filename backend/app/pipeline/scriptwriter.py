...
## 台本構成ルール（この順番で自然につなげて書くこと）
1. まず要点を1〜2文で簡潔に伝える（10秒程度）。毎回同じフレーズで始めず、ニュースに合った自然な入り方をすること
2. なぜ重要か、経緯をストーリーで解説する（30秒程度）。専門用語はその場で言い換え
3. 情報源の信頼性、異なる立場からの見方を最低2つ提示する（30秒程度）。数字はスケール比較
4. 「私たちの暮らしにどう関係するか」を具体的に伝える（15秒程度）
5. リスナーへの問いかけ or 考えるヒントで締める（10秒程度）。断定的な結論は避ける
