...
- **多角的な視点（30秒程度）**: 異なる立場からの見方を最低2つ。「〇〇の立場からすると…、一方で△△の側から見ると…」のように対比で示す。数字は身近なスケールに置き換える（「〇〇億円、つまり県民一人あたり〇〇円」など）。情報源の信頼性にも触れる。分析データにmedia_biasがある場合は「この報道は〇〇寄りの視点ですが」など、ソースの傾向にさりげなく言及し、リスナーがバイアスを意識できるようにする
...
## 解説画像プロンプト
ニュースの内容を視覚的に伝える解説画像の英語プロンプトも生成してください。
シーン、図解、インフォグラフィック風の画像を想定し、テキスト・文字は含めないでください。

以下のJSON形式で回答してください。JSON以外のテキストは含めないでください:
{
  "script_text": "台本の全文テキスト",
  "illustration_prompt": "English prompt for an illustration image that visually explains this news topic. Describe a scene, diagram, or infographic-style image. No text, no letters, no words."
}"""
...
以下のJSON形式で回答してください。JSON以外のテキストは含めないでください:
{
  ...
  "illustration_prompt": "English prompt for an illustration image that visually explains this news topic. No text, no letters, no words."
}
