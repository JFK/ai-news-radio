...
async def _run_step(args: dict) -> str:
    ...
    video_targets = args.get("video_targets")

    step = await client.run_step(
        episode_id, step_name, queries,
        tts_model=tts_model, tts_voice=tts_voice, video_targets=video_targets,
    )
